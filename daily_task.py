...
def start_proxy():
    """启动 sing-box 本地代理，返回 http://127.0.0.1:8080 或 None"""
    if not PROXY_STR:
        log("🌐 未配置代理，直连模式")
        return None

    # http/socks5 直接返回给浏览器用
    p = urlparse(PROXY_STR.strip())
    if p.scheme in ("http", "https", "socks5", "socks4"):
        log(f"🌐 使用直连代理: {p.scheme}://{p.hostname}:{p.port}")
        return f"{p.scheme}://{p.hostname}:{p.port}"

    config_json = generate_config(PROXY_STR)
    if not config_json:
        log("⚠️ 代理格式无法识别，尝试直连")
        return None
    ...
    try:
        ...
        log("❌ sing-box 未安装，尝试直连")
        return None
    except Exception as e:
        log(f"⚠️ 代理验证失败({e})，仍尝试使用")
        return "http://127.0.0.1:8080"
...
async def build_agent(page, key_index=0):
    ...
    agent_config = AgentConfig(
        ...
        IMAGE_CLASSIFIER_MODEL="gemini-3.5-flash-lite",
        SPATIAL_POINT_REASONER_MODEL="gemini-3.5-flash-lite",
        SPATIAL_PATH_REASONER_MODEL="gemini-3.5-flash-lite",
        EXECUTION_TIMEOUT=240,
        RESPONSE_TIMEOUT=90,
        RETRY_ON_FAILURE=True,
        # 挑战视图渲染慢（代理网络），加大渲染等待避免截图超时
...
async def solve_hcaptcha(page, agent_holder, scene="page", max_attempts=5):
    """用 hcaptcha-challenger 自动解 hCaptcha（多轮挑战有竞态，靠多次重试覆盖）"""
    for attempt in range(1, max_attempts + 1):
        ...
        # 若挑战弹窗还开着（上轮残留），先点刷新开新题，避免 frame 竞态
        challenge_open = await page.evaluate("""
            () => !!document.querySelector('iframe[src*="hcaptcha.com"][src*="frame=challenge"]')
        """)
        if challenge_open and attempt > 1:
            try:
                await agent_holder["agent"].robotic_arm.refresh_challenge()
                log("  🔄 已刷新挑战，开新题")
    ...
    return bool(await get_token(page))
...
async def do_task(page, email):
    ...
    task_btn = page.locator('xpath=//button[contains(.,"Renew")]')
    try:
